...
Здесь описана архитектура проекта, создание кастомных тем, добавление звуков, работа с профилями, сборка `.exe` и публикация релизов.
...
9. [Публикация релиза на GitHub (OTA-обновления)](#9-публикация-релиза-на-github-ota-обновления)
10. [Конфигурационные ключи](#10-конфигурационные-ключи)
...
AppVersion=1.3.0                          ; ← обновлять при каждом релизе
...
## 9. Публикация релиза на GitHub (OTA-обновления)

Система OTA-обновлений работает через **GitHub Releases API**.  
При запуске приложение делает запрос к:
```
GET https://api.github.com/repos/krinix1337/KeySoundOverlay/releases/latest
```
И сравнивает `tag_name` с `CURRENT_VERSION`.

### Пошаговая инструкция выпуска обновления

**Шаг 1 — Обновить код и собрать**
```bash
# Обновить версию (3 файла — см. раздел 8.4)
# Собрать установщик
build.bat
# Скомпилировать setup.iss в Inno Setup → dist/KeySoundOverlay_Setup.exe
```

**Шаг 2 — Запушить изменения**
```bash
git add -A
git commit -m "Release 1.4.0: описание изменений"
git push origin main
```

**Шаг 3 — Создать GitHub Release**

1. Перейди на [github.com/krinix1337/KeySoundOverlay/releases](https://github.com/krinix1337/KeySoundOverlay/releases)
2. Нажми **Draft a new release**
3. Заполни:

| Поле | Значение |
|---|---|
| **Tag version** | `v1.4.0` (обязательно с `v`) |
| **Release title** | `KeySound Overlay v1.4.0` |
| **Description** | Список изменений (поддерживает Markdown) |
| **Attach files** | Прикрепи `dist/KeySoundOverlay_Setup.exe` |

4. Нажми **Publish release** ✅

> [!NOTE]
> После публикации релиза все пользователи с включённой проверкой обновлений при следующем запуске получат предложение обновиться.

---

### Формат описания релиза (Markdown)

```markdown
## 🆕 Что нового в v1.4.0

### ✨ Новые функции
- Добавлена тёмная тема "Midnight"
- Новый свич: Boba U4T (тактильный тихий)

### 🐛 Исправления
- Исправлен crash при старте без APPDATA
- Устранена утечка памяти в ripple-анимации

### ⚙️ Улучшения
- Уменьшена задержка звука на 15%
- Улучшен детектор полноэкранного режима

**Полный список изменений:** https://github.com/krinix1337/KeySoundOverlay/compare/v1.3.0...v1.4.0
```

---

## 10. Конфигурационные ключи
...
### GitHub API выдаёт 404 при проверке обновлений

Убедись, что в `app/update_manager.py` правильный репозиторий:
```python
DEFAULT_GITHUB_REPO = "krinix1337/KeySoundOverlay"
```
И что в репозитории существует хотя бы один опубликованный **Release** с тегом вида `v1.x.x`.

---

<p align="center">
  <b>KeySound Overlay v1.3.0</b><br>
